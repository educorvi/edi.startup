...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_number(self, page_count):
        ...
        if self._pageNumber == page_count:
            self.drawString(1.2 * cm, 3.2 * cm, "educorvi GmbH & Co. KG")
            self.drawString(1.2 * cm, 2.4 * cm, "Karolinenstraße 17")
            ...
            self.drawString(9.25 * cm, 3.2 * cm, "Geschäftsführer: Lars Walther")
            ...
            self.drawString(9.25 * cm, 1.2 * cm, "Finanzamt Fürth")
            self.drawRightString(20 * cm, 3.2 * cm, "Seite %d von %d" % (self._pageNumber, page_count))
...
# Invoice details in separate row
    ...
    colWidths = [1 * cm, 12 * cm, 1.5 * cm, 3 * cm]
    services = _("Services")
    hours = _("Hours")
    subtotal = _("Subtotal")
